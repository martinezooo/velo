...
- **Network calls must be bounded**: every outbound HTTP request goes through `utils/fetchWithTimeout.ts`. `navigator.onLine` only reports whether an interface exists, so on a captive portal, a dropped VPN, or a router with no upstream route a plain `fetch` opens a socket and never settles — the sync bar then sits at "Syncing…" forever with no error to surface. `classifyError` already treats the resulting timeout as retryable
- **AI task visibility**: AI calls run through `runAiTask()` from `stores/aiStatusStore.ts` so `AiActivityIndicator` can show in-flight and failed states. Never let an AI call fail silently, and never auto-retry one from a `useEffect` whose guard is "result is still null" — that spins
- **Message body theme**: HTML mail is authored for white backgrounds, so `EmailRenderer` keeps it light regardless of the app theme. `uiStore.emailBodyTheme` lets the reader force dark, which inverts the body and re-inverts media; plain text always follows the app theme
