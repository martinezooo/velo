...
export function ThreadView({ thread }: ThreadViewProps) {
  // Always the thread's own account: in "All inboxes" the open thread may not
  // belong to the account selected in the sidebar.
  ...
  useEffect(() => {
    if (!accountId || thread.isRead || markedReadRef.current === threadKey) return;
    if (markAsReadBehavior === "manual") return;

    const markRead = () => {
      markedReadRef.current = threadKey;
    ...
    // instant
    markRead();
  }, [accountId, threadKey, thread.id, thread.isRead, updateThread, markAsReadBehavior]);
  ...
  return (
    <div className="flex h-full @container relative">
      ...
      {contactSidebarVisible && primarySender && accountId && (
        <>
